...
public class courseScheduler {
    ...
    public void rescheduleConflicts() {
        if (!coursesToBeRescheduled.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (courseMap.get(unscheduledCourseUUID).numberOfSessions == courseMap
                        .get(unscheduledCourseUUID).sessionsScheduled) {
    ...
    public void addCourseHandleLectures(UUID courseUUID) {
        ...
        if (courseMap.get(courseUUID).numberOfSections > 1) {
            ...
            for (int i = 1; i <= courseMap.get(courseUUID).numberOfSections; i++) {
                ...
                course currentSection = new course(courseMap.get(courseUUID).courseID + "-" + i,
                        courseMap.get(courseUUID).courseName,
                        ...
                        courseMap.get(courseUUID).instructorDays, courseMap.get(courseUUID).TimeSlotIndexstart,
                        courseMap.get(courseUUID).TimeSlotIndexEnd, courseMap.get(courseUUID).conflictingCourses,
                        courseMap.get(courseUUID).courseType, courseMap.get(courseUUID).nbOfSlots,
                        courseMap.get(courseUUID).duration);
    ...
    public void addCourse(UUID courseUUID) {
        ...
        if (attemptDayPairSchedule(courseUUID)
                && !(courseMap.get(courseUUID).numberOfSessions < courseMap.get(courseUUID).instructorDays.size())) {
    ...
    private boolean attemptDayPairSchedule(UUID courseId) {
        ...
        for (List<Integer> dayPair : availableDayPairs) {
            ...
            while (courseMap.get(courseId).sessionsScheduled < pairSessions
                    && timeSlotIndex <= course.TimeSlotIndexEnd - course.nbOfSlots) {
                boolean canSchedule = true;
                for (int session = 0; session < course.nbOfSlots
                        && courseMap.get(courseId).sessionsScheduled < pairSessions; session++) {
    ...
    private boolean attemptEqualSpreadSchedule(UUID courseId) {
        ...
        for (int dayIndex : courseMap.get(courseId).instructorDays) {
            ...
            int sessionsPerDay = courseMap.get(courseId).numberOfSessions
                    / courseMap.get(courseId).instructorDays.size();
            int sessionsScheduled = 0;

            for (int i = courseMap.get(courseId).TimeSlotIndexstart; i < courseMap.get(courseId).TimeSlotIndexEnd
                    && sessionsScheduled < sessionsPerDay; i++) {
                if (isSlotAvailable(courseId, dayIndex, i)) {
                    if (courseMap.get(courseId).nbOfSlots > 1) { // case 1, each course lecture takes more than 1 slot
                                                                 // of time
    ...
    private boolean attemptAnySchedule(UUID courseId) {
        ...
        for (int dayIndex : courseMap.get(courseId).instructorDays) {
            for (int i = courseMap.get(courseId).TimeSlotIndexstart; i < courseMap.get(courseId).TimeSlotIndexEnd
                    && courseMap.get(courseId).sessionsScheduled < courseMap.get(courseId).numberOfSessions; i++) {
                if (isSlotAvailable(courseId, dayIndex, i)) {
                    if (courseMap.get(courseId).sessionsScheduled >= courseMap.get(courseId).numberOfSessions) {
                        break;
                    }
                    if (courseMap.get(courseId).nbOfSlots > 1
                            && areSlotsAvailable(courseId, dayIndex, i, i + courseMap.get(courseId).nbOfSlots - 1)) {
                        scheduleCourseInSlots(courseId, dayIndex, i, i + courseMap.get(courseId).nbOfSlots - 1);
    ...
    private boolean isSlotAvailable(UUID courseUUID, int dayIndex, int slotIndex) {
        ...
        for (String conflict : courseMap.get(courseUUID).conflictingCourses) {
            for (UUID scheduledCourseUUID : schedule[dayIndex][slotIndex]) {
                ...
                if (courseMap.get(scheduledCourseUUID).instructorName
                        .equals(courseMap.get(courseUUID).instructorName)) {
                    return false;
                }

                String scheduledCourseid = courseMap.get(scheduledCourseUUID).courseID;
                // parsing to get rid of -i of different sections
                // for example instead of comparing PHYS201-2 (present in slot) which isnt
                // present in the course.conflictingCourses
                // we compare PHYS201
                String[] scheduledCourseSplit = scheduledCourseid.split("-");
    ...
    // checks for conflicts in several slots in a row, used for courses with longer
    // than 1 slot lecture time
    ...
    private List<Integer> getNextDayPair(int lectureIndex) {
        // Assumes dayPairs is a list containing pairs of day indices, e.g., [[0, 2],
        // [1, 3], [2, 4]]
