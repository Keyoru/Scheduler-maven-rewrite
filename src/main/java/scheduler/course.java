...
public class course {
   ...
   int numberOfSections; // if 2 sections and 2 sessions per week then each section has 2 sessions per
                         // week
   ...
   int nbOfSlots; // number of slots a single lecture takes
                  // if course lecture length is 1:15 then 1 slot, 2 hours length is 2 slots etc
                  // etc
   ...
   course(String ID, String name, int creds, int sections, int sessions, String instname,
         LinkedList<Integer> instdays, int index1, int index2, LinkedList<String> conflicts, String Type, int Slots,
         String Duration) {
      courseID = ID;
      courseName = name;
